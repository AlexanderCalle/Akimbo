...
const RecentPost = ({ article, idx }) => {
  ...
  return (
    <section
      className={`w-full lg:w-96 h-96 lg:h-fit flex flex-col gap-4 items-center justify-start`}
    >
      ...
      {article.image !== "" ? <img
        ...
        className="w-full h-3/6 aspect-square object-cover bg-center"
      /> : <div className="w-full h-3/6 aspect-square object-cover bg-center" style={{backgroundColor: article.bg_color}} />}
      <article
        className={`w-full flex flex-col p-2 z-10 items-start gap-2 bg-akimbo-light bg-opacity-80 backdrop-blur-sm`}
      >
        ...
        <p
          className={`w-full h-2/3 overflow-hidden text-ellipsis text-start`}
