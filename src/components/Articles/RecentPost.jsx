...
const RecentPost = ({ article, idx }) => {
  ...
  return (
    <section className="max-w-96 flex flex-col gap-4 items-start justify-start">
      <div className="self-start font-sans m-1 px-3 py-1 absolute bg-akimbo-light bg-opacity-70 lowercase">{article.type === TYPE_ARTICLE ? article.cat : article.type}</div>
       {article.image !== "" ? <img
        ...
        className="w-full min-h-[150px] h-2/6 object-cover bg-center"
      /> : <div className="w-full h-3/6 object-cover bg-center" style={{backgroundColor: article.bg_color}} />}
      <article
        className={`h-96 flex flex-col p-2 items-start justify-between gap-2 bg-akimbo-light bg-opacity-80 backdrop-blur-sm`}
      >
        ...
        <button
          className="bg-akimbo-dark-900 px-3 py-2 text-akimbo-light justify-self-end"
          onClick={() => {
